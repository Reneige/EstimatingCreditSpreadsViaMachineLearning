...
class database_builder:
    def __init__(self):
        ...
        # glob2 returns lists of file locations based on search strings. 
        # The below are the locations of the data files to aggregate 
        ...
        # counts the number of files to feed to the progress bar
        self.total_files = len(
            self.price_files) + len(self.financial_files) + len(self.bond_universe) + 4
        
        # specify database location, instantiate progress bar and run code
    ...
    def read_price(self, file):
        ...
        identifier = pd.read_excel(file, index_col=None, 
                                   usecols="A", header=3, nrows=0)
    ...
    def read_financials(self, file):
        ...
        findata_dict = pd.read_excel(file, sheet_name=None, 
                                     skiprows=10, index_col=0)

        # extract company name from data and add as dataframe column
        company = pd.read_excel(file, index_col=None,
                                usecols="B", header=1, nrows=0)
    ...
    def build_database(self, pricedata, security_master, findata, nomcurve, inflcurve, gdp, vix, ftse100):
        ...
        pricedata.to_sql('Prices',  conn, 
            if_exists='replace', index=False)
        
        security_master.to_sql('Master', conn, 
            if_exists='replace', index=False)
    
        findata.to_sql('Financials', conn, 
            if_exists='replace', index=False)
        
        nomcurve.to_sql('Nominal_Curve', conn,
            if_exists='replace', index=False)
        
        inflcurve.to_sql('Inflation_Curve', conn,
            if_exists='replace', index=False)
    ...
    def run(self):
        ...
        list_of_dfs3 = list(
            map(self.read_bond_universe_master, self.bond_universe))
        ...
        self.progressbar.change_message(
            "Injecting into database - please be patient ")
        self.progressbar.progress_step()

        # build database
        self.build_database(pricedata, security_master,
                            findata, nomcurve, inflcurve, gdp, vix, ftse100)
