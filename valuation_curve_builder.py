...
class interpolate_curve:
    ...
    def build_database(self, curvedata):
        ...
        self.progressbar.change_message("Injecting data into Database - Please be patient")
        ...
        self.progressbar.change_message("Creating Database Search Index - Please be patient")
    ...
    def interpolate_spot_curve(self):
        ...
        for date in valuation_dates:
            ...
            # add the valuation date to periods - background: previously the curve would begin 1 month after valuation date
            # this makes it satrt from day 0 which is required for coupons expiring less than a month away
            ...
            # insert the first element of list into index 0 (i.e. duplicate it). This aligns the data 
        ...
        # collapse list of dataframe into final large dataframe, rename index, convert from timestamp to datetime.date
        finaldata = pd.concat(dflist)
        finaldata = finaldata.reset_index().rename(columns={'index':'spot_date'})
        finaldata['spot_date'] = list(map(timestamp_to_date,(finaldata['spot_date'])))
        return finaldata 
